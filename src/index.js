...
module.exports = postcss.plugin('avoid-globals', userConfig => {
    ...
    const validateRoot = root => {
        ...
        if(!excluded) {
             root.walkRules(rule => validateRule(rule));
        }
    }
     ...
     */
    const validateRule = (rule, filename) => {
        rule.selector.split(/\s+/)
            .forEach((selector, index) => validateSelector(selector, index, rule));
     ...
     */
    const validateSelector = (selector, depth, rule) => {
        ...
        if (typeof selectorType === 'string' && !config[scope][selectorType]) {
            console.warn(`🐱 Please don't use CSS selectors of type "${selectorType}" in ${scope} scope (${position} "${selector}").`);
        }

        // Inform the user about using an invalid selector type.
        if (typeof selectorType !== 'string' && scope === 'global') {
            console.warn(`🐶 Found invalid CSS rule (${position} "${rule.selector}").`);
        }
    }
